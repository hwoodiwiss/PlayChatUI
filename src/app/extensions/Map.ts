...
declare global {
  export interface Map<K, V> {
    ...
    first(
      predicate?: (key: K, val: V) => boolean
    ): { key: K; value: V } | undefined;
  }
}

Map.prototype.where = function <K, V>(
  this: Map<K, V>,
  predicate: (key: K, val: V | [K, V]) => boolean
): Map<K, V> {
...
Map.prototype.select = function <K, V, T>(
  this: Map<K, V>,
  predicate: (key: K, val: V) => T
): Array<T> {
...
Map.prototype.first = function <K, V>(
  this: Map<K, V>,
  predicate?: (key: K, val: V | [K, V]) => boolean
): { key: K; value: V } | undefined {
  for (const item of this) {
